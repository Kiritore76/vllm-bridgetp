...
"""Freeze the exact workload proven by a passing CAP-0 Safe abandon bring-up."""
...
from tools.bridge_tp.run_phase9_cap0_abandon import (  # noqa: E402
    validate_abandon_pressure,
)
...
def require_passing_acceptance(
    ...
    if int(acceptance.get("capacity_decisions", 0)) != 1:
        errors.append("bring-up does not contain exactly one capacity decision")
    if int(acceptance.get("start_shadow_decisions", 0)) != 1:
        errors.append("bring-up does not contain exactly one START_SHADOW")
    if int(acceptance.get("shadow_capacity_clear_samples", 0)) < 1:
        errors.append("bring-up has no causal capacity CLEAR during SHADOW")
    if int(acceptance.get("cleanup_complete_events", 0)) != 1:
        errors.append("bring-up does not contain exactly one completed cleanup")
    if acceptance.get("trigger_path") != "CAPACITY_PILOT":
        errors.append("bring-up did not use the CAPACITY_PILOT trigger")
    if acceptance.get("transition_states") != ["SHADOW", "CANCELLED"]:
        errors.append("bring-up abandon path is incomplete or illegal")
    if acceptance.get("final_state") != "CANCELLED":
        errors.append("bring-up did not finish in CANCELLED")
    if acceptance.get("takeover_state") != "CANCELLED":
        errors.append("bring-up takeover state is not CANCELLED")
    if acceptance.get("source_abort_dispatched") is not False:
        errors.append("bring-up dispatched a source abort")
    if acceptance.get("source_continues_on_tp1") is not True:
        errors.append("bring-up did not preserve TP1 ownership")
    if acceptance.get("source_cleanup_status") != "CLEANED":
        errors.append("bring-up source cleanup is incomplete")
    if acceptance.get("stager_cleanup_status") != "CLEANED":
        errors.append("bring-up stager cleanup is incomplete")
    if acceptance.get("forbidden_artifacts"):
        errors.append("bring-up contains post-cutover artifacts")
    if int(acceptance.get("receiver_receipt_count", -1)) != 0:
        errors.append("bring-up contains receiver receipts")
    emitted = int(acceptance.get("anchor_emitted_tokens", 0))
    source = int(acceptance.get("source_origin_tokens", 0))
    target = int(acceptance.get("target_origin_tokens", -1))
    if emitted != expected_anchor_tokens:
        errors.append("bring-up emitted an unexpected anchor length")
    if source != emitted or target != 0:
        errors.append("bring-up output was not exclusively owned by TP1")
    if errors:
...
def build_frozen_manifest(
    ...
    paths = {
        ...
        "acceptance": root / "provenance" / "abandon_acceptance.json",
    ...
    frozen["scenario"] = "CAP-0 Safe abandon formal"
...
def main() -> None:
    args = parse_args()
    if os.name == "nt":
        raise RuntimeError("CAP-0 Safe abandon freezing requires Linux")
    if subprocess.run(
    ...
    print(f"FROZEN_ABANDON_MANIFEST={args.out.resolve()}")
    print(f"FROZEN_ABANDON_SHA256={frozen_sha}")
    print(f"FROZEN_ABANDON_PROVENANCE={provenance.resolve()}")
