...
"""Run three or more reportable CAP-0 Safe abandon repetitions."""
...
from tools.bridge_tp import run_phase9_cap0_abandon as abandon  # noqa: E402
...
from tools.bridge_tp.freeze_phase9_cap0_abandon import (  # noqa: E402
...
def validate_bringup_contract(args: argparse.Namespace) -> dict[str, Any]:
    ...
    acceptance_path = root / "provenance" / "abandon_acceptance.json"
    ...
    if manifest.get("scenario") != "CAP-0 Safe abandon formal":
...
def validate_inputs(
    ...
    if args.repetitions < 3:
        raise ValueError("formal Safe abandon requires at least three repetitions")
    revision, guard, pressure = abandon.validate_inputs(
        args,
        expected_scenario="CAP-0 Safe abandon formal",
    )
...
def formal_contract(
    ...
    return {
        "format_version": 1,
        "scenario": "CAP-0 Safe abandon formal",
        "revision": revision,
...
def main() -> None:
    ...
    try:
        for repetition in range(1, args.repetitions + 1):
            ...
            print(
                f"[{repetition}/{args.repetitions}] starting formal Safe abandon",
                flush=True,
            )
            result = scenario_runner.run(
                ...
                run_id=run_id,
                scenario="abandon",
                scenario_title="CAP-0 Safe abandon formal",
                provenance_status="FORMAL_REPETITION",
                platform_note=(
                    "CAP-0 SAFE ABANDON FORMAL; automated 5x A100 PCIe"
                ),
                success_status="PASS",
                success_marker="PASS",
                acceptance_fn=partial(
                    abandon.accept_abandon,
        ...
        common.write_json(status_path, batch)
        print(f"ABANDON_FORMAL_COMPLETE: {out_root}", flush=True)
