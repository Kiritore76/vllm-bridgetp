...
"""Run three or more reportable CAP-0 Rescue repetitions."""
...
from tools.bridge_tp.freeze_phase9_cap0_rescue import (  # noqa: E402
...
def validate_bringup_contract(args: argparse.Namespace) -> dict[str, Any]:
    ...
    acceptance_path = root / "provenance" / "rescue_acceptance.json"
    ...
    if manifest.get("scenario") != "CAP-0 Rescue formal":
...
def validate_inputs(
    ...
    if args.repetitions < 3:
        raise ValueError("formal Rescue requires at least three repetitions")
    revision, guard, pressure = rescue.validate_inputs(
        args,
        expected_scenario="CAP-0 Rescue formal",
    )
...
def main() -> None:
    ...
    try:
        for repetition in range(1, args.repetitions + 1):
            ...
            print(
                f"[{repetition}/{args.repetitions}] starting formal Rescue",
                flush=True,
            )
            result = scenario_runner.run(
                ...
                run_id=run_id,
                scenario="rescue",
                scenario_title="CAP-0 Rescue formal",
                provenance_status="FORMAL_REPETITION",
                platform_note=(
                    "CAP-0 RESCUE FORMAL; automated 5x A100 PCIe"
                ),
                success_status="PASS",
                success_marker="PASS",
                acceptance_fn=rescue.accept_rescue,
        ...
        common.write_json(status_path, batch)
        print(f"RESCUE_FORMAL_COMPLETE: {out_root}", flush=True)
